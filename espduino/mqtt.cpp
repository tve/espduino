...
boolean MQTT::begin(const char* client_id, const char* user, const char* pass, uint16_t keep_alive, boolean clean_seasion) {
  ...
  crc = esp->request(CMD_MQTT_SETUP, 0, 1, 10);
...
boolean MQTT::begin(const __FlashStringHelper* client_id, const __FlashStringHelper* user, const __FlashStringHelper* pass, uint16_t keep_alive, boolean clean_seasion) {
  uint16_t crc;
  uint32_t wait, cb_data;

  crc = esp->request(CMD_MQTT_SETUP, 0, 1, 10);
  crc = esp->request(crc, client_id, strlen_P((const char*)client_id));
  crc = esp->request(crc, user, strlen_P((const char*)user));
  crc = esp->request(crc, pass, strlen_P((const char*)pass));
  crc = esp->request(crc, (uint8_t*)&keep_alive, 2);
  crc = esp->request(crc, (uint8_t*)&clean_seasion, 1);
...
boolean MQTT::lwt(const __FlashStringHelper* topic, const __FlashStringHelper* message, uint8_t qos, uint8_t retain) {
  uint16_t crc;
  uint32_t wait;
  crc = esp->request(CMD_MQTT_LWT, 0, 1, 5);
  crc = esp->request(crc, (uint8_t*)&remote_instance, 4);
  crc = esp->request(crc, topic, strlen_P((const char*)topic));
  crc = esp->request(crc, message, strlen_P((const char*)message));
  crc = esp->request(crc, (uint8_t*)&qos, 1);
  crc = esp->request(crc, (uint8_t*)&retain, 1);
  esp->request(crc);
  if (esp->waitReturn() && esp->return_value)
...
boolean MQTT::lwt(const __FlashStringHelper* topic, const __FlashStringHelper* message) {
  return lwt(topic, message, 0, 0);
}
...
void MQTT::connect(const __FlashStringHelper* host, uint32_t port, boolean security) {
  ...
  crc = esp->request(CMD_MQTT_CONNECT, 0, 0, 4);
  crc = esp->request(crc, (uint8_t*)&remote_instance, 4);
  crc = esp->request(crc, host, strlen_P((const char*)host));
  crc = esp->request(crc, (uint8_t*)&port, 4);
  crc = esp->request(crc, (uint8_t*)&security, 1);
  esp->request(crc);
}
...
void MQTT::subscribe(const __FlashStringHelper* topic, uint8_t qos) {
  uint16_t crc;
  crc = esp->request(CMD_MQTT_SUBSCRIBE, 0, 0, 3);
  crc = esp->request(crc, (uint8_t*)&remote_instance, 4);
  crc = esp->request(crc, topic, strlen_P((const char*)topic));
...
void MQTT::subscribe(const __FlashStringHelper* topic) {
  subscribe(topic, 0);
}

void MQTT::publish(const char* topic, const uint8_t* data, const uint16_t len, uint8_t qos, uint8_t retain) {
  uint16_t crc;
...
void MQTT::publish(const __FlashStringHelper* topic, const __FlashStringHelper* data, const uint16_t len, uint8_t qos, uint8_t retain) {
  uint16_t crc;
  crc = esp->request(CMD_MQTT_PUBLISH, 0, 0, 6);
  crc = esp->request(crc, (uint8_t*)&remote_instance, 4);
  crc = esp->request(crc, topic, strlen_P((const char*)topic));
  crc = esp->request(crc, data, len);
  crc = esp->request(crc, (uint8_t*)&len, 2);
  crc = esp->request(crc, (uint8_t*)&qos, 1);
  crc = esp->request(crc, (uint8_t*)&retain, 1);
  esp->request(crc);
...
void MQTT::publish(const char* topic, const __FlashStringHelper* data, const uint16_t len, uint8_t qos, uint8_t retain) {
  uint16_t crc;
  crc = esp->request(CMD_MQTT_PUBLISH, 0, 0, 6);
  crc = esp->request(crc, (uint8_t*)&remote_instance, 4);
  crc = esp->request(crc, (uint8_t*)topic, strlen(topic));
...
void MQTT::publish(const __FlashStringHelper* topic, const uint8_t* data, const uint16_t len, uint8_t qos, uint8_t retain) {
  uint16_t crc;
  crc = esp->request(CMD_MQTT_PUBLISH, 0, 0, 6);
  crc = esp->request(crc, (uint8_t*)&remote_instance, 4);
  crc = esp->request(crc, topic, strlen_P((const char*)topic));
...
void MQTT::publish(const char* topic, const char* data, uint8_t qos, uint8_t retain) {
...
void MQTT::publish(const __FlashStringHelper* topic, const __FlashStringHelper* data) {
  publish(topic, data, strlen_P((const char*)data), 0, 0);
}

void MQTT::publish(const char* topic, const __FlashStringHelper* data) {
  publish(topic, data, strlen_P((const char*)data), 0, 0);
}

void MQTT::publish(const __FlashStringHelper* topic, const char* data) {
  publish(topic, (const uint8_t*)data, strlen(data), 0, 0);
}
